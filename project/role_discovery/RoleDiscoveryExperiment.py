...
def analyze_role_characteristics(data: torch.Tensor, role_labels: torch.Tensor, model_name: str, dataset_name: str, k: int, output_dir: Path):
    ...
    role_summary_df.to_csv(analysis_save_path)
...
def run_hyperparameter_tuning(dataset_name: str):
    ...
    for model_name in param_grid.keys():
        ...
        if not model_results.empty:
            ...
            if model_name == 'GNN_Embedder_GAE':
                best_model = GNNEmbedder(**best_params, model_path=str(model_save_path), force_retrain=True)
            else: 
                best_model = DGIEmbedder(in_channels=data.num_features, **best_params, model_path=str(model_save_path), force_retrain=True)
...
def main():
    ...
    parser.add_argument('--dataset', type=str, nargs='+', default=['Cora'],
                        help="A list of datasets to run on or summarize, e.g., --dataset Cora Actor.")
